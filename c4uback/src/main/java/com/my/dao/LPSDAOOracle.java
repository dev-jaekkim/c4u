...
public class LPSDAOOracle implements LPSDAO {
	...
	try {
		...
		List<Student> list = session.selectList("mybatis.lpsMapper.selectBylessonId", map);
		if(list.size() == 0) {
			throw new FindException("학생정보가 없습니다.");
		}
		return list;
	}catch (Exception e) {
	...
	public int selectByStudentAllLessonCnt(int studentId) throws FindException {
		...
		try {
			session = sqlSessionFactory.openSession();			
			int AllCount = session.selectOne("mybatis.lpsMapper.selectByStudentAllLessonCnt",studentId);
			return AllCount;
		} catch (Exception e) {
			throw new FindException(e.getMessage());
		} finally {
	...
	public List<Lesson> selectByPageStudentLesson(int currentPage, int cnt_per_page, int studentId) throws FindException {
		...
		try {	
			...
			map.put("lps_student_id", studentId);
			map.put("currentPage", currentPage);
			map.put("cnt_per_page", cnt_per_page);
			List<Lesson> list = session.selectList("mybatis.lpsMapper.selectByPageStudentLesson", map);
			if(list.size() == 0) {
				throw new FindException("학생이 수강한 강좌가  없습니다.");
			}
			return list;
			}catch(Exception e) {
				throw new FindException(e.getMessage());
	...
	@Override
	public List<Lesson> selectByPage(int cnt_per_page, int currentPage, int studentId) throws FindException {
		return null;
	}
	...
	@Override
	public int selectAllCount(int studentId, int lessonStatus) throws FindException {
	...
	@Override
	public List<Lesson> selectByPage(int currPage, int dataPerPage, int studentId, int lessonStatus)
