...
public interface ILessonReviewService {
	//2021-04-29 김재경: 관리자용 메서드 추가
		 ...
		 */
		List<LessonReview> findPerPage(int currentPage, int cnt_per_page) throws FindException;
		 ...
		 * @throws FindException 수강후기가 없으면, 예외가 발생한다. 
		 */
		List<LessonReview> findPerPage(int lessonId, int currentPage, int cnt_per_page) throws FindException;
		 ...
		 */
		List<LessonReview> findByLessonTitleORStudentNamePerPage(String word, 
																   int currentPage, 
		 ...
		 */
		void add(LessonReview lessonReview) throws AddException;
		 ...
		 */
		int remove(int review_id) throws RemoveException;
}